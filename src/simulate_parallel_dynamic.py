...
def run_task(task):
    ...
    grid, mask, max_iter, tol = task
    return jacobi_solver(grid, mask, max_iter, tol)
...
if __name__ == '__main__':
    ...
    # Dynamic scheduling: use imap with chunksize=1 so tasks are assigned as workers become free
    worker_count = cpu_count()
    with Pool(processes=worker_count) as pool:
        simulation_results = list(pool.imap(run_task, simulation_tasks, chunksize=1))
